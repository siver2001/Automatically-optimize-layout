...
async function run() {
  ...
  })).filter(shape => shape.sizeName === '9' || shape.sizeName === '12.5');

  console.log(`Verifying fixes for Size 9 and Size 12.5...`);

  for (const sizeInfo of testSizes) {
    const sizeName = sizeInfo.sizeName;
    console.log(`\n-------------------------------------`);
    console.log(`Testing Size ${sizeName}...`);
      ...
      for (let j = i + 1; j < items.length; j++) {
        ...
        if (itemA.isSplit && itemA.cycPolygon) {
          ...
          if (cachedPolygonsOverlap(
            ...
            console.log(`  [DIE-TO-MATERIAL OVERLAP] Split ${itemA.id}'s die overlaps with material of ${itemB.id}!`);
            overlapsCount++;
          }
        }

        // 3. Die B vs Material A
        if (itemB.isSplit && itemB.cycPolygon) {
          ...
          if (cachedPolygonsOverlap(
            ...
            console.log(`  [DIE-TO-MATERIAL OVERLAP] Split ${itemB.id}'s die overlaps with material of ${itemA.id}!`);
            overlapsCount++;
          }
        }
      }
