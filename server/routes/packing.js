...
import PackingOrchestrator from '../services/packingOrchestrator.js';
...
router.post('/optimize-batch', async (req, res) => {
  try {
    ...
    // Basic validation
    if (!container || !rectangles || !quantities) {
    ...
    const onProgress = (progress) => {
      const chunk = JSON.stringify({ type: 'progress', value: progress }) + '\n';
      res.write(chunk);
    };

    const result = await PackingOrchestrator.optimizeBatch(
      container,
      rectangles,
      quantities,
    ...
    console.log(`Batch optimization completed in ${duration}ms`);
