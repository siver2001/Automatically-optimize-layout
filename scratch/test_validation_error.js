...
// Buggy version from frontend
function polygonDistanceBuggy(a = [], b = []) {
  ...
  for (let i = 0; i < b.length; i++) {
    const b1 = b[i],
      b2 = b[(j + 1) % b.length]; // Typos 'j' here!
...
function buildSheetValidation(sheet, spacing, useFixed = false) {
  const invalid = new Set();
  const rawItems = sheet?.placed || [];
  const templates = sheet?.renderTemplates || {};
  
  // Reconstruct polygon for each item as frontend does
  ...
  for (let i = 0; i < items.length; i++) {
    for (j = i + 1; j < items.length; j++) {
      ...
      const distanceFunc = useFixed ? polygonDistanceFixed : polygonDistanceBuggy;
      
      try {
        if (distanceFunc(aa.polygon || [], bb.polygon || []) + 0.02 < gap) {
          invalid.add(aa.id);
          invalid.add(bb.id);
        }
      } catch (err) {
        console.error(`Error calculating distance between ${aa.id} and ${bb.id}:`, err.message);
      }
    }
  }
...
async function run() {
  ...
  console.log('Running test capacity for Size 8...');
  const res = await engine.testCapacity([size8], config);
  const sheet = res.sheetsBySize['8'];

  // Simulate compactSheetForCapacity (strip polygon)
  const compactSheet = {
    ...sheet,
  ...
  console.log('\n--- Running with BUGGY polygonDistance on Compact Sheet ---');
  const resultBuggy = buildSheetValidation(compactSheet, 3, false);
  console.log('Invalid items count:', resultBuggy.invalidCount);
  console.log('Invalid items:', Array.from(resultBuggy.invalidItemIds));

  console.log('\n--- Running with FIXED polygonDistance on Compact Sheet ---');
  const resultFixed = buildSheetValidation(compactSheet, 3, true);
  console.log('Invalid items count:', resultFixed.invalidCount);
  console.log('Invalid items:', Array.from(resultFixed.invalidItemIds));
}
